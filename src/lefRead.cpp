...
int layerCB(lefrCallbackType_e c, lefiLayer* _layer, lefiUserData) {
    ...
    if (_layer->lefiLayer::hasDirection()){
        ...
        if(direction == "HORIZONTAL") 
            newLayer.direction = PrefDir::HORIZONTAL;
        else if(direction == "VERTICAL") 
            newLayer.direction = PrefDir::VERTICAL;
        else
        {
            cout << "????????????" << endl;
            exit(0);
...
int pinCB(lefrCallbackType_e c, lefiPin* _pin, lefiUserData) {
    ...
    if (_pin->lefiPin::hasDirection())
    {
        ...
        /*
        cout <<_pin->lefiPin::direction() << endl;
        if(string(_pin->lefiPin::direction()) == "INPUT")
            newPin.direction = PortDir::INPUT;
        if(string(_pin->lefiPin::direction()) == "OUTPUT")
            newPin.direction = PortDir::OUTPUT;
        */
    ...
    if (_pin->lefiPin::hasUse()) 
    {
        ...
        //cout << newPin.use << " " << _pin->lefiPin::use() << endl;


        /*
        if(_pin->lefiPin::use() == "CLOCK")
        {
            newPin.use = UseType::CLOCK;
            cout << "!" << endl;
        }
        else if(_pin->lefiPin::use() == "SIGNAL")
        {
            newPin.use = UseType::SIGNAL;
            cout << "!" << endl;
        }
        else if(_pin->lefiPin::use() == "GROUND")
        {
            newPin.use = UseType::GROUND;
            cout << "!" << endl;
        }
        else if(_pin->lefiPin::use() == "POWER")
        {
            newPin.use = UseType::POWER;
            cout << "!" << endl;
        }
        */
    }
...
int PowerOpt::readLEF(const char* input) {
